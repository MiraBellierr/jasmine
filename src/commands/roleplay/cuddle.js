...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const count = await getCount(message, target, "cuddle");
    ...
    const embed = new Discord.EmbedBuilder()
      ...
      .setColor("#CD1C6C")
      .setFooter({
        text: `${target.user.username} has been cuddled by ${message.author.username} ${count} times!`,
      });

    message.reply({ embeds: [embed] });
  },
  interaction: roleplayInteraction("cuddle", "cuddle someone", { target: "required", command: () => module.exports }),
};

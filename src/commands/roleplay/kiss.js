...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const count = await getCount(message, target, "kiss");
    ...
    const embed = new Discord.EmbedBuilder()
      ...
      .setColor("#CD1C6C")
      .setFooter({
        text: `${target.user.username} has been kissed by ${message.author.username} ${count} times!`,
      });

    message.reply({ embeds: [embed] });
  },
  interaction: roleplayInteraction("kiss", "kiss someone", { target: "required", command: () => module.exports }),
};

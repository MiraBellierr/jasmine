...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const count = await getCount(message, target, "stare");
    ...
    const embed = new Discord.EmbedBuilder()
      ...
      .setColor("#CD1C6C")
      .setFooter({
        text: `${target.user.username} has been stared by ${message.author.username} ${count} times!`,
      });

    message.reply({ embeds: [embed] });
  },
  interaction: roleplayInteraction("stare", "stare at someone", { target: "required", command: () => module.exports }),
};

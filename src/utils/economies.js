...
const getTimer = async (user) => {
  let timers = await schemas.timer().findOne({ where: { userID: user.id } });

  if (!timers) {
    timers = await schemas.timer().create({
      userID: user.id,
    });
  }
...
const getCooldown = async (user, property, timeout) => {
  const ms = await import("parse-ms");
  const timers = await getTimer(user);

  const time = timers.get(property);

  if (time !== null && timeout - (Date.now() - time) > 0) {
    const timeObj = ms.default(timeout - (Date.now() - time));
